...
The script `triage_client.py` is used to automatically download malware samples from `bazaar.abuse.ch`, then analyze them using `triage` sandbox and download the corresponding pcap files and reports. 
...
Script `dataset_creator.py` is used if you want to create your own new dataset from Suricata log files and malware samples.

And finally the script `capture.py` is used for live capture and classification of network flow records generated by Suricata tool.

## Instalation

Firstly you need to install all the necessary python packages using pip:
```
...
If you want to start a live capture and classification of real time flows, you need to install Suricata tool first. The guide for instalation is described here [https://suricata.readthedocs.io/en/suricata-6.0.0/quickstart.html](https://suricata.readthedocs.io/en/suricata-6.0.0/quickstart.html).
...
Then just turn off the suricata deamon, if it's running using command: 
```
...
This archive contains `src` folder with all necessary modules. Then it contains four main python scripts, configuration files, two datasets in `csv` files and folder `out/reports` with few results of dynamic malware analysis, to demonstrate IOC's capture during live classification of flows. This folder is here so, you don't need to obtain other results of dynamic analysis. 

## Usage of main scripts

This next part shortly describes each individual main script. It describes it arguments and shows some examples how to run them with corresponding outputs.
...
If option `--verbose` is used, the script will print info about every classified flow. It prints if it was normal or malicious. If the flow is classified as malicous, than source and destination adress and port will be also printed.

If option `--verbose` isn't used the program will only print total info about the classified flow in given period of time (standart is 3 minutes). After that the flow cnt of normal and malicious flows will start again. When the program is finnished the total statistics about classified flows is printer.
